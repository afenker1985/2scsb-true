...
class Eleventy {
  ...
  logFinished() {
    if (!this.writer) {
      throw new Error(
        "Did you call Eleventy.init to create the TemplateWriter instance? Hint: you probably didnâ€™t."
      );
    }
...
Arguments:
  ...
  async _watch() {
    ...
    if (this.watchManager.getPendingQueueSize() > 0) {
      console.log(
        `You saved while Eleventy was running, letâ€™s run again. (${this.watchManager.getPendingQueueSize()} remain)`
      );
      await this._watch();
    } else {
  ...
  async _initWatchDependencies() {
    ...
    // Deps from Global Data (that arenâ€™t in the global data directory, everything is watched there)
    this.watchTargets.addDependencies(
